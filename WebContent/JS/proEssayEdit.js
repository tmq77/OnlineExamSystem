$(function(){
	...
	$(".modifypro").on("click",function(){
		...
		var essayId = id;
		var essayRight = $("#pro_"+id).val();
		var essayTitle = $.trim($("#title_"+id).val());
		$.confirm({
			icon:"glyphicon glyphicon-cog",
			title:"修改试题",
			type:"blue",
			content:"<label>专业简答</label>"+
				...
				ok:{
					...
					action:function(){
						...
						$.ajax({
							...
							cache:false,
							url:"ModifyProEssay",
							data:"essayId="+essayId+"&essayTitle="+this.$content.find(".essayTitle").val()+"&essayRight="+this.$content.find(".essayRight").val()+"&post="+$("#postselect").val(),
							success:function(data){
								if(data=="none"){
									$.alert({
				...
				ok:{
					...
					action:function(){
						$.ajax({
							type:"post",
							url:"DeleteProEssay",
							data:"essayId="+id+"&post="+$("#postselect").val(),
							dataType:"text",
							cache:false,
							success:function(data){
								if(data=="none"){
	...
	$("#before").on("click",function(){
		$.ajax({
			type:"POST",
			url:"GetAllProEssay",
			data:"pageNo="+$(this).attr("name")+"&post="+$("#postselect").val(),
			cache:false,
			dataType:"text",
			success:function(data){
				$("#content").html(data);
	...
	if($.trim($("#textpageNo").val()).length==0){
		...
		$.ajax({
			type:"POST",
			url:"GetAllProEssay",
			data:"pageNo="+$("#textpageNo").val()+"&post="+$("#postselect").val(),
			dataType:"text",
			success:function(data){
				$("#content").html(data);
			},
