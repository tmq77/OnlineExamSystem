$(function(){
	...
	$("#subSearch").on("click",function(){
		...
		var type = $("#type").val();
		var post = $("#postrequired").val();
		$.ajax({
			url:"Search",
			data:"str="+$("#search").val()+"&type="+type+"&post="+post,
			dataType:"text",
			cache:false,
	...
	$(".modifyrequired").on("click",function(){
		var id = $(this).attr("id").substring(2);
		var essayId = id;
		var essayRight = $("#re_"+id).val();
		var essayTitle  = $.trim($("#title_"+id).val());
		$.confirm({
			type:"blue",
			title:"修改试题",
			icon:"glyphicon glyphicon-cog",
			content:"<label>必答题</label>"+
				...
				ok:{
					...
					action:function(){
						...
						$.ajax({
							...
							dataType:"text",
							cache:false,
							url:"ModifyRequiredQs",
							data:"rid="+essayId+"&title="+this.$content.find(".essayTitle").val()+"&rright="+this.$content.find(".essayRight").val()+"&post="+$("#postrequired").val(),
							success:function(data){
								if(data=="none"){
									$.alert({
				...
				buttons:{
					ok:{
						...
						action:function(){
							$.ajax({
								type:"post",
								url:"DeleteRequiredQs",
								data:"rid="+id+"&post="+$("#postrequired").val(),
								dataType:"text",
								cache:false,
								success:function(data){
									if(data=="none"){
	...
	$("#before").on("click",function(){
		$.ajax({
			...
			data:"pageNo="+$(this).attr("name")+"&post="+$("#postrequired").val(),
			cache:false,
			dataType:"text",
			success:function(data){
				$("#content").html(data);
	...
	if($.trim($("#textpageNo").val()).length==0){
		...
		$.ajax({
			type:"POST",
			url:"GetAllRequiredQs",
			data:"pageNo="+$("#textpageNo").val()+"&post="+$("#postrequired").val(),
			dataType:"text",
			success:function(data){
				$("#content").html(data);
			},
